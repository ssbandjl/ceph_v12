...
Gil::Gil(SafeThreadState &ts, bool new_thread) : pThreadState(ts)
{
  ...
  dout(25) << "GIL acquired for thread state " << pThreadState.ts << __FFL__ << dendl;
  ...
  if (new_thread) {
    ...
    dout(20) << "Switched to new thread state " << pNewThreadState << __FFL__ << dendl;
  } else {
...
Gil::~Gil()
{
  if (pNewThreadState != nullptr) {
    dout(20) << "Destroying new thread state " << pNewThreadState << __FFL__ << dendl;
  ...
  PyEval_SaveThread();
  dout(25) << "GIL released for thread state " << pThreadState.ts << __FFL__ << dendl;
}
